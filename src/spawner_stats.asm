...
@LJMP   0x004DCB7C,  _HouseClass__Tracking_Remove_New_Planes_Tracking
...
@LJMP   0x004DCB5B,  _HouseClass__Tracking_Remove_New_Building_Tracking
...
gbool OutOfSync, false

sstring MapHashField, "HASH"
sstring MyIdField, "MYID"
sstring OutOfSyncField, "OOSY"

    
hack 0x0052A8BD, 0x0052A8C3 ; check if ESC was pressed on the recon dialog
    cmp ax, 0x1B
    jne 0x0052A8CA ; return false
...
hack 0x0052BBAB ; out of sync
    mov byte[OutOfSync], 1
    mov ebx, 0x14E
    jmp hackend

hack 0x005B65AA ; fix up SDFX
    xor ebx, ebx
    mov bl, byte[ConnectionLost]
    jmp hackend
    
hack 0x005B65C3 ; add OOSY Field (out of sync)
    xor ebx, ebx
    mov bl, byte[OutOfSync]
    mov eax, 0x10
    call OperatorNew
    test eax, eax
    jz .fail
    mov edx, OutOfSyncField
    call 0x005B86D8 ; FieldClass::FieldClass(char *,ulong)
...
hack 0x005B6801 ; add MYID field for the cncnet ladder
    xor ebx, ebx
    mov eax, dword[HouseClass__PlayerPtr]
    mov bl, byte[eax+1]
    sub ebx, 12
...
hack 0x005B6829 ; add HASH field for the cncnet ladder
    lea ebx, [MapHash]
    mov eax, 0x10
    call OperatorNew
    ...
    mov edx, MapHashField
    call FieldClass__FieldClass_String
    
.fail:
    mov edx, eax
    lea eax, [ebp-0x6C]
    call PacketClass__Add_Field

    mov eax, 0x10
    jmp hackend


hack 0x004CAAC8 ; set resign bool to true when abort button was clicked
    push eax
    mov eax, dword[HouseClass__PlayerPtr]
    mov dword[eax+EXT_Resigned], 1
    pop eax
...
_Send_Statistics_Packet_Send_Only_Once:
	cmp		BYTE [_SessionClass__Session], 0 ; if single player dont send statistics
	jz		.Early_Ret
...
.First_Time:    
    ...
    cmp dword[0x006ABC18], 1 ;GameTimerInUse
    jnz .Normal_Code
...
;    cmp     BYTE [_SessionClass__Session], 4
;    jmp     0x004A0B07
